...
function operator(y) {
  ...
  y.forEach((res) => {
    ...
    for (const elem of Object.keys(inputList)) {
      if (res.name.indexOf(elem) !== -1) {
        let namekey = nf ? jcname + FGF : "";
        if (addflag) {
          ikey.push(gF(us[Object.keys(inputList).indexOf(elem)]) + FGF + namekey + outputList[Object.keys(inputList).indexOf(elem)]);
        } else {
          ikey.push(outputList[Object.keys(inputList).indexOf(elem)]);
        }
        if (bl) {
