...
delete_by_rule_number() {
    while true; do
        ...
        rule_details=$(sudo ufw status numbered | grep -E "^\[\s*${rule_number}\s*\]" || echo "")

        if [ -z "$rule_details" ]; then
            echo -e "${RED}错误: 未找到规则序号 $rule_number${NC}"
            sleep 2
            continue
        fi
